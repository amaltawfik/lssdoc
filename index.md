# lssdoc: LimeSurvey questionnaires to Word and PDF, and back
...
- **`"table"`** – one dense table covering the whole document: every
  ...
  ![Table layout: one variable per row with English and French columns
...
| `as_lss_spec(lss)` | Turn a survey read with [`read_lss()`](https://amaltawfik.github.io/lssdoc/reference/read_lss.md) into a specification. |
...
[`audit_lss()`](https://amaltawfik.github.io/lssdoc/reference/audit_lss.md)
catches without supplying your own file:
...
An existing survey can enter the same loop:
[`write_form_docx()`](https://amaltawfik.github.io/lssdoc/reference/write_form_docx.md)
renders it as the form, so a questionnaire can be exported from
LimeSurvey, edited in Word and re-imported. See
[`vignette("write-in-word")`](https://amaltawfik.github.io/lssdoc/articles/write-in-word.md)
for the conventions, the question types and the filter syntax.
...
MIT. See [`LICENSE`](https://amaltawfik.github.io/lssdoc/LICENSE) for
the full notice.
