...
- **Variable-centric compound rendering.** Array, multiple-choice
  and multiple-numerical questions are documented per subquestion,
  with the real variable code (`parent_subq`) on the meta row -- not
  the parent code, which never appears in the data export. Dual-scale
  arrays go one step further: each subquestion is split into one
  single-choice block per scale (`parent_subq_1` / `parent_subq_2`,
  the 1-based scale index LimeSurvey uses in its data export), so
  every column of the data export is documented on its own.
