...
* `render_questionnaire()` produces a Word or PDF questionnaire document
  ...
  extension; PDF is produced locally via LibreOffice. Page orientation
  follows the template, not the language count: the `cards` template is
  laid out in A4 portrait and the dense `table` template in A4 landscape
  by default; `page_format` overrides this with `"A4-portrait"`,
  `"A4-landscape"` or `"A3"`, and every panel scales to the chosen width.
...
* `show_toc`, `show_index` and `show_header_title` (all `TRUE`) add a
  ...
  indented beneath it, quotas, variable index -- each a clickable link
  to the matching heading, rather than only the question groups. Group
  headings themselves are not numbered. The page footer always carries
  a compact `X/Y` page counter.
