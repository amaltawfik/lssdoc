...
* New experimental authoring layer: `lss_spec()` builds and deeply validates
  ...
  The specification is language-aware: `languages = c("fr", "en")` declares
  the survey languages and every text (title, welcome and end texts, group
  titles, question texts and help, option labels, quota names and messages)
  can be given per language as a named vector or list, but this version
  emits the primary language only -- multilingual emission follows in 0.3.0.
  The emitted format was validated by importing a generated 21-question
  survey, covering every supported question kind, into LimeSurvey
  7.0.0-beta1 (build 260121), which imported the DBVersion 700 file
  without warnings and re-exported it with every question, subquestion,
  answer, attribute and relevance equation intact (DBVersion still 700).
